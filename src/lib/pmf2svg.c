...
int U_pmf_onerec_draw(const char *contents, const char *blimit, int recnum,
    ...
    case (U_PMR_DRAWIMAGE):
        // Skip EMF+ DRAWIMAGE - use EMF STRETCHDIBITS instead for correct positioning
        // U_PMR_DRAWIMAGE_draw(contents, out, states);
        break;
    case (U_PMR_DRAWIMAGEPOINTS):
        // Skip EMF+ DRAWIMAGEPOINTS - use EMF STRETCHDIBITS instead for correct positioning
        // The EMF+ image records often contain duplicate images at different coordinates
        // and can have cross-contamination issues when converting multiple EMF files
...
/**
  \brief Store data from a  U_PMF_IMAGE object for later drawing
  \return 1 on success, 0 on error
  \param  contents   Record from which to extract data
  \param  objectId   Object ID to store this image under
...
int U_PMF_IMAGE_draw(const char *contents, FILE *out, drawingStates *states,
    ...
    // Allocate plus objects if not done yet
    if (!states->plusObjects) {
        states->plusObjects = (emfPlusObjects *)calloc(1, sizeof(emfPlusObjects));
        if (!states->plusObjects)
            return 0;
    }

    // Validate object ID
    if (objectId < 0 || objectId >= MAX_EMFPLUS_OBJECTS)
        return 0;

    // Get image header
    status = U_PMF_IMAGE_get(contents, &Version, &Type, &Data,
                             (const char *)states->endAddress);
    if (!status)
        return 0;

    // Clear any previous image at this slot
    emfPlusImage *img = &states->plusObjects->images[objectId];
    if (img->data) {
        free(img->data);
        img->data = NULL;
    }
    memset(img, 0, sizeof(emfPlusImage));

    if (Type == U_IDT_Bitmap) {
        // Parse bitmap data
        status = U_PMF_BITMAP_get(Data, &Bs, &BitmapData,
                                  (const char *)states->endAddress);
        if (!status)
            return 0;
        ...
        img->stride = Bs.Stride;
        img->pixelFormat = Bs.PxFormat;
        img->bitmapType = Bs.Type;

        // For compressed images (PNG, JPEG, etc), Bs.Type == 1
        // The data starts right after the header and extends to end of object
        if (Bs.Type == 1) {
            // Compressed image - estimate size (we need the actual size)
            // Data points to start of image data
            // Calculate approximate size based on typical EMF+ structure
            size_t dataSize = Bs.Width * Bs.Height * 4; // max estimate
            // Better: use stride if available
            if (Bs.Stride > 0) {
                dataSize = (size_t)Bs.Stride * (size_t)Bs.Height;
            }
            // For compressed, it's typically much smaller - need actual size
            // We'll try to detect based on PNG/JPEG headers
            if (BitmapData[0] == (char)0x89 && BitmapData[1] == 'P' &&
                BitmapData[2] == 'N' && BitmapData[3] == 'G') {
                // PNG - scan for IEND chunk to find actual size
                // For now, use a reasonable estimate
                dataSize = 0;
                const char *p = BitmapData + 8;
                while (p < (const char *)states->endAddress - 12) {
                    uint32_t chunkLen = ((uint8_t)p[0] << 24) | ((uint8_t)p[1] << 16) |
                                        ((uint8_t)p[2] << 8) | (uint8_t)p[3];
                    if (chunkLen > 0x1000000) break; // sanity check
                    p += 4; // skip length
                    if (p[0] == 'I' && p[1] == 'E' && p[2] == 'N' && p[3] == 'D') {
                        dataSize = (p + 8) - BitmapData; // +4 type +4 crc
                        break;
                    }
                    p += 4 + chunkLen + 4; // type + data + crc
                }
            } else if (BitmapData[0] == (char)0xFF && BitmapData[1] == (char)0xD8) {
                // JPEG - scan for EOI marker (0xFF 0xD9)
                const char *p = BitmapData + 2;
                while (p < (const char *)states->endAddress - 1) {
                    if (p[0] == (char)0xFF && p[1] == (char)0xD9) {
                        dataSize = (p + 2) - BitmapData;
                        break;
                    }
                    p++;
                }
            ...
            if (dataSize > 0 && dataSize < 0x10000000) {
                img->data = (char *)malloc(dataSize);
                if (img->data) {
                    memcpy(img->data, BitmapData, dataSize);
                    img->dataSize = dataSize;
                }
            }
        } else {
            // Raw bitmap - Type == 0 (MSBitmap)
            size_t dataSize = (size_t)Bs.Stride * (size_t)Bs.Height;
            if (dataSize > 0 && dataSize < 0x10000000) {
                img->data = (char *)malloc(dataSize);
                if (img->data) {
                    memcpy(img->data, BitmapData, dataSize);
    ...
    } else if (Type == U_IDT_Metafile) {
        // Nested metafile - not supported for now
        img->valid = false;
    }
...
/**
  \brief Draw data from a  U_PMR_DRAWIMAGE record
  \return size of record in bytes on success, 0 on error
  \param  contents   Record from which to extract data
...
int U_PMR_DRAWIMAGE_draw(const char *contents, FILE *out,
    ...
    int ctype;
    int32_t SrcUnit;
    U_PMF_RECTF SrcRect, DstRect;
    int status;
    char *b64Data = NULL;
    size_t b64Size;
    double posX, posY, sizeW, sizeH;

    if (!contents || !out || !states)
        return 0;

    status = U_PMR_DRAWIMAGE_get(contents, NULL, &ImgID, &ctype, &ImgAttrID,
                                 &SrcUnit, &SrcRect, &DstRect);
    if (!status)
        return 0;

    // Check if we have the image stored
    if (!states->plusObjects || ImgID >= MAX_EMFPLUS_OBJECTS)
        return 1; // Silently skip - no image stored

    emfPlusImage *img = &states->plusObjects->images[ImgID];
    if (!img->valid || !img->data || img->dataSize == 0)
        return 1; // Silently skip - no valid image

    // Calculate position - just apply scaling
    // The reference offset is handled by the SVG group transform
    posX = (double)DstRect.X * states->scaling;
    posY = (double)DstRect.Y * states->scaling;
    sizeW = (double)DstRect.Width * states->scaling;
    sizeH = (double)DstRect.Height * states->scaling;

    // Determine image MIME type
    const char *mimeType = "image/png"; // default
    if (img->bitmapType == 1 && img->dataSize > 2) {
        // Compressed image - detect type from header
        if ((unsigned char)img->data[0] == 0x89 && img->data[1] == 'P' &&
            img->data[2] == 'N' && img->data[3] == 'G') {
            mimeType = "image/png";
        } else if ((unsigned char)img->data[0] == 0xFF &&
                   (unsigned char)img->data[1] == 0xD8) {
            mimeType = "image/jpeg";
        } else if (img->data[0] == 'G' && img->data[1] == 'I' && img->data[2] == 'F') {
            mimeType = "image/gif";
        }
    } else {
        // Raw bitmap - would need conversion to PNG
        // For now, skip raw bitmaps (requires complex conversion)
        return 1;
    }

    // Base64 encode the image data
    b64Data = base64_encode((unsigned char *)img->data, img->dataSize, &b64Size);
    if (!b64Data)
        return 1;

    // Output SVG image element
    fprintf(out, "<%simage x=\"%.4f\" y=\"%.4f\" width=\"%.4f\" height=\"%.4f\" ",
            states->nameSpaceString, posX, posY, sizeW, sizeH);
    fprintf(out, "xlink:href=\"data:%s;base64,%s\" ", mimeType, b64Data);
...
/**
  \brief Draw data from a  U_PMR_DRAWIMAGEPOINTS record
  \return size of record in bytes on success, 0 on error
  \param  contents   Record from which to extract data
...
int U_PMR_DRAWIMAGEPOINTS_draw(const char *contents, FILE *out,
                               drawingStates *states) {
    uint32_t ImgID, ImgAttrID, Elements;
    ...
    status = U_PMR_DRAWIMAGEPOINTS_get(contents, NULL, &ImgID, &ctype, &etype,
                                       &RelAbs, &ImgAttrID, &SrcUnit, &SrcRect,
                                       &Elements, &Points);
    if (!status) {
    ...
    // DrawImagePoints uses 3 points to define a parallelogram destination
    // Points[0] = upper-left, Points[1] = upper-right, Points[2] = lower-left
    // We'll compute a bounding rectangle for simpler rendering
    double minX, minY, maxX, maxY;
    if (Elements >= 3 && Points) {
        minX = Points[0].X;
        maxX = Points[0].X;
        minY = Points[0].Y;
        maxY = Points[0].Y;

        for (uint32_t i = 1; i < Elements && i < 3; i++) {
        ...
        // Calculate fourth point (lower-right) for parallelogram
        double p3x = Points[0].X + (Points[1].X - Points[0].X) + (Points[2].X - Points[0].X);
        double p3y = Points[0].Y + (Points[1].Y - Points[0].Y) + (Points[2].Y - Points[0].Y);
    ...
    // Calculate position - just apply scaling
    // The reference offset is handled by the SVG group transform
    double posX = minX * states->scaling;
    double posY = minY * states->scaling;
    double sizeW = (maxX - minX) * states->scaling;
    double sizeH = (maxY - minY) * states->scaling;
...
int U_PMR_OBJECT_draw(const char *contents, const char *blimit,
    ...
    if (status) {
        ...
        case U_OT_Region:
            ...
            break;
        case U_OT_Image:
            (void)U_PMF_IMAGE_draw(ObjCont->accum, out, states, ObjCont->Id);
            break;
        case U_OT_Font:
