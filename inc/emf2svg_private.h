...
// Maximum number of EMF+ objects per slot
#define MAX_EMFPLUS_OBJECTS 256

// EMF+ Image object storage
typedef struct {
    bool valid;
    uint32_t type;       // U_IDT_Bitmap or U_IDT_Metafile
    uint32_t width;
    uint32_t height;
    uint32_t stride;
    uint32_t pixelFormat;
    uint32_t bitmapType; // 0=MSBitmap, 1=compressed (PNG/JPG/etc)
    char *data;          // Raw image data (owned by this struct)
    size_t dataSize;
} emfPlusImage;

// EMF+ objects table (indexed by object ID)
typedef struct {
    emfPlusImage images[MAX_EMFPLUS_OBJECTS];
} emfPlusObjects;
